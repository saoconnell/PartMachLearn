...
system.time(
  pp.svm.FIT <- train(classe ~ ., data = harTrain,
                       method = "svmRadial",
                       trControl = harControl,
                       verbose = FALSE)
)

save(pp.svm.FIT, file="data/pp_svm_FIT.Rdata")

pp.svm_yHat <- predict(pp.svm.FIT, harTest)

confusionMatrix(harTest$classe, pp.svm_yHat)
...
FINAL.pp.svm_yHat <- predict(pp.svm.FIT, pp.test.transformed)

as.character(FINAL.pp.svm_yHat)


PROB.pp.svm_yHat <- predict(pp.svm.FIT, pp.test.transformed)

heatmap(as.matrix(FINAL.rd.nnet_yHat), Rowv=NA, Colv=NA)